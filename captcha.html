...
<body>
	...
	<script>
		...
		function reloadCaptcha() {
			var captchaImg = document.getElementById("captchaImg");
			if (captchaImg) {
				captchaImg.src = "captcha.php?" + new Date().getTime(); // Adiciona um parâmetro de data para recarregar a imagem
		...
		function generateCaptcha() {
			...
			if (captchaImg) {
				...
				captchaImg.src = "captcha.php?" + new Date().getTime(); // Atualiza o src da imagem gerada
			}
			document.getElementById("captchaInput").value = ""; // Limpa o campo de input
		}
